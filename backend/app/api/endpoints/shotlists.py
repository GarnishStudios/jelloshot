...
def read_shotlists(
    ...
    shotlists = shotlist_service.get_project_shotlists(db, project_id=project_id, skip=skip, limit=limit)
    return shotlists
...
def create_shotlist(
    ...
    return shotlist_service.create_shotlist(db=db, shotlist=shotlist, project_id=project_id)
...
def update_shotlist(
    ...
    db_project = project_service.get_project(db, project_id=db_shotlist.project_id)
    if db_project.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Not authorized to update this shotlist")

    return shotlist_service.update_shotlist(db=db, shotlist_id=shotlist_id, shotlist=shotlist)
...
def delete_shotlist(
    ...
    db_project = project_service.get_project(db, project_id=db_shotlist.project_id)
    if db_project.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Not authorized to delete this shotlist")
